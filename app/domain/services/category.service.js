...
class CategoryService {
	...
	async getCategories(getAllRequest){
		...
		const categories = await this.categoryRepository.findAllWithFilterAndPagination(
	...
	async createCategory(categoryData){
		const category = await this.categoryRepository.findOne({name: categoryData.name});
		if (category) {
